...
% TODO  we want to define 'metacircular evaluator' in the context of this project for people not familiar with theoretical computer science or programing language implementation.  The main point is explaining why we're approaching it this way instead of as a conventional compiler / interpreter. My perspective is that it's important to treat the environment / environment frame as a first class data structure that is directly accessible and modifiable by evaluator / host language. Consider that we \emph{need} to represent the environment as a structured object when we pass it to the associative memory system. It's ok to make this discussion longish, up to a page. You can include sicp style code snippets to explain concepts, if you want.
% TODO we also want to define 'environment' and note that it's equivalent to lexical scope from the implemented language's perspective
% TODO we also want to define staged compilation and connect it to what we're doing here
...
%;; Stage 2: Parse XML into uniform AST
%(define (parse-task xml)
%  (match xml
%    [(node operator args ...)
%     (make-ast-node 
...
%;; Stage 1: LLM generates XML task structure
%(define xml
%  (llm-translate query))
%
%;; Stage 2: Parse XML into uniform AST
%(define (parse-task xml)
%  (match xml
%    ;; Pattern: match a node that has an operator and any number of args
%    ;; (node operator args ...) matches XML structure like:
%    ;; <node>
%    ;;   <operator>...</operator>
%    ;;   <arg>...</arg>
%    ;;   <arg>...</arg>
...
%    ;; </node>
%    [(node operator args ...)
%     (make-ast-node (parse-operator operator)
%                    (map parse-task args))]
%    
%    ;; Pattern: match a leaf node (no children)
%    ;; Just return the leaf value directly
%    [leaf leaf]))
%
%;; Stage 3: Tree traversal and execution
%(define (execute ast env)
%  (let ((operator (node-operator ast))
%        (args (node-args ast)))
%    (apply operator 
%           (map (lambda (arg) (execute arg env))
%                args)
%           env)))
%\end{lstlisting}
...
<node>
  <!-- Map operation over multiple inputs -->
  <operator>
    {"type": "map",
     "operation": "analyze_peaks"}
  </operator>
...
%
%"Look through the experiment logs and tell me when we saw unexpected beam behavior."
%
%The LLM first translates this natural language query into a structured task:
%
%\begin{lstlisting}[language=XML]
%<node>
%  <operator>
%    {"type": "compound", "task": "analyze_logs"}
%  </operator>
%  <arg>
%    <node>
%      <operator>
...
%
%\begin{lstlisting}[language=Python]
%# Simple AST showing task structure
%node = ASTNode(
%    operator=Operator(
%        type="atomic",
%        task="extract_patterns",
...
%\end{lstlisting}
%
%Finally, the evaluator executes this with environment handling:
%
%\begin{lstlisting}[language=Python]
%# Environment contains the actual log contents
%env = Environment(context={
%    "logs": [
%        "10:15 - Beam current: 3.2mA, within normal range",
%        "10:23 - Warning: Unexpected beam loss detected",
%        "10:24 - Beam current dropped to 1.1mA",
%        # ... more log entries
%    ]
%})
%
%# LLM execution 
%result, env = evaluator.execute_llm(node.operator, env)
%# LLM analyzes logs through natural language understanding
%# Returns findings about beam behavior
%\end{lstlisting}
%
%This example shows how the LLM serves both roles:
%1. As compiler: Interpreting natural language intent into structured tasks
%2. As executor: Reading and analyzing log content in context
...
%(Lexical scope refers to the standard nesting behavior of variable bindings -- for example:
%\begin{lstlisting}[language=Scheme]
%(let ((x 1))          ; x=1 in this scope
%  (let ((y 2))        ; y=2 in this nested scope
...
<node>
  <operator>
    {"type": "atomic", "task": "analyze_region_quality"}
  </operator>
  <arg>
    <!-- First argument will be output of log parsing -->
    <node>
      <operator>
        {"type": "atomic", 
         "task": "extract_stats",
         "params": {
           "instruction": "Find background region parameters and p-values"
         }}
      </operator>
    </node>
  </arg>
